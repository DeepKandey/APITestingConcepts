...
public class CreateJSONUsingPOJO {
	...
	private void createJSONUsingPOJOtest() throws JsonProcessingException {
		...
		// POJO as JSON
		String employeeJSON = objectMapper.writerWithDefaultPrettyPrinter().writeValueAsString(employeeObjAsRequest1);
		System.out.println("\nEmployee POJO as JSON: \n" + employeeJSON);

		// POJO as JSON Array using ArrayNode
		ArrayNode node = objectMapper.createArrayNode().addPOJO(employeeObjAsRequest1).addPOJO(employeeObjAsRequest2)
				.addPOJO(employeeObjAsRequest3);
		String employeeJSONArray = objectMapper.writerWithDefaultPrettyPrinter().writeValueAsString(node);
		...
		// (Serialize)
		String employeeListJSONArray = objectMapper.writerWithDefaultPrettyPrinter()
				.writeValueAsString(allEmployeesAsRequest);
		...
		List<Employee> allEmployeesAsResponse = objectMapper.readValue(employeeListJSONArray,
